...
public class RiversController {
	...
	public void setModel(Model model) {
		...
		try {
			boxRiver.getItems().addAll(model.getRiversMap().values());
			boxRiver.valueProperty().addListener(new ChangeListener<River>() {
				
				public void changed(ObservableValue<? extends River> observable, River oldRiver, River newRiver) {
					txtStartDate.setText(model.getStartDate(newRiver));
					txtEndDate.setText(model.getEndDate(newRiver));
					txtNumMeasurements.setText(model.getNumMeasurements(newRiver));
					txtFMed.setText(model.getFMed(newRiver));
				}
